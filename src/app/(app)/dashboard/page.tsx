...
async function StaffDashboard() {
  const { disputeQueue, pendingRedemptions, ticketQueue, verificationQueue } =
...
async function SuperAdminDashboard() {
  ...
  return (
    <div className="space-y-8">
      <section className="grid gap-4 md:grid-cols-3">
        <StatCard
          hint="All pickup requests created in the system so far."
          icon={PackageCheck}
          label="Total pickups"
          value={opsSummary.pickupCount}
        />
        <StatCard
          hint="Requests that have completed verification successfully."
          icon={ShieldCheck}
          label="Verified pickups"
          value={opsSummary.verifiedCount}
        />
        <StatCard
